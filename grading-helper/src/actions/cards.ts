...
/**
 * セットコードからTCGゲーム種別を判定する
 * ワンピース: OP-XX, ST-XX, EB-XX, P-XXX（3桁数字プロモ）
 * それ以外はポケモンとして扱う
 */
function detectTcgGame(setCode: string): string {
  const code = setCode.toUpperCase();
  if (/^(OP|ST|EB)-\d+/.test(code)) return "onepiece";
  if (/^P-\d{3,}$/.test(code)) return "onepiece";
  return "pokemon";
}

/**
 * パック名または任意の文字列から年（西暦）を抽出する
 * 例: "マクドナルド ハッピーセット2025" → 2025
 */
...
export async function importCardMasterBatch(
  ...
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    const records = batch.map((row) => {
      ...
      const year = row.year
        ?? (row.packName ? extractYearFromText(row.packName) : null)
        ?? null;

      // ポケモンカードのみ英語名・grading_name を生成
      let nameEn: string | null = null;
      let gradingName: string | null = null;

      if (tcgGameId === "pokemon" && row.cardName) {
        ...
        if (matched) {
          nameEn = megaPrefix
            ...
            : matched.name_en;
          if (extracted.suffix) nameEn += ` ${extracted.suffix}`;
        }

        // grading_name は年がある場合のみ生成
        if (year && nameEn) {
          const result = generateGradingName(
            {
              setNumber: `${row.setCode}-${row.cardNumber}`,
