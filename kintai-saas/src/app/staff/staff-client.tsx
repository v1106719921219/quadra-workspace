...
export function StaffClient({ employees }: { employees: Employee[] }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // 月が変わった場合の対応: 選択なしでも保存（既存を削除）
      const dummyDate = `${year}-${String(month).padStart(2, "0")}-01`;
      await saveDayOffRequests(selectedEmployee.id, dates.length > 0 ? dates : [dummyDate]);
      if (dates.length === 0) {
        // 全削除の場合は空で保存
          ...
          {calendarDays.map((dateStr, i) => {
            ...
            return (
              <button
                ...
                disabled={loading}
                className={`
                  rounded-lg flex flex-col items-center justify-start pt-1 text-sm font-medium transition-colors min-h-[52px]
